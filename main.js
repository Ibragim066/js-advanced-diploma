...
var Character = /*#__PURE__*/function () {
  function Character(level) {
    ...
    if ((this instanceof Character ? this.constructor : void 0) === Character) throw new Error('Недопустимый класс персонажа');
  }
  _createClass(Character, [{
    key: "showInformation",
    value: function showInformation() {
      return "\uD83C\uDF96 ".concat(this.level, " \u2694 ").concat(this.attack, " \uD83D\uDEE1 ").concat(this.defence, " \u2764 ").concat(this.health);
    }
  }, {
    key: "travelRadiusIndex",
    value: function travelRadiusIndex(index, boardSize) {
      var box = [];
      var numberRow = Math.floor(index / boardSize);

      // left
      for (var i = 1; i <= this.travelRadius; i += 1) {
        var result = index - i;
        if (result < numberRow * boardSize) break;
        box.push(result);
      }
      // right
      for (var _i = 1; _i <= this.travelRadius; _i += 1) {
        var _result = index + _i;
        if (_result >= (numberRow + 1) * boardSize) break;
        box.push(_result);
      }
      // up
      for (var _i2 = 1; _i2 <= this.travelRadius; _i2 += 1) {
        var _result2 = index - _i2 * boardSize;
        if (_result2 < 0) break;
        box.push(_result2);
      }
      // down
      for (var _i3 = 1; _i3 <= this.travelRadius; _i3 += 1) {
        var _result3 = index + _i3 * boardSize;
        if (_result3 > Math.pow(boardSize, 2) - 1) break;
        box.push(_result3);
      }
      // up-left
      for (var _i4 = 1; _i4 <= this.travelRadius; _i4 += 1) {
        var _result4 = index - _i4 * (boardSize + 1);
        if (_result4 < 0 || _result4 === boardSize * (numberRow - _i4) - 1) break;
        box.push(_result4);
      }
      // up-right
      for (var _i5 = 1; _i5 <= this.travelRadius; _i5 += 1) {
        var _result5 = index - _i5 * (boardSize - 1);
        if (_result5 < 0 || _result5 === boardSize * (numberRow - _i5 + 1)) break;
        box.push(_result5);
      }
      // down-left
      for (var _i6 = 1; _i6 <= this.travelRadius; _i6 += 1) {
        var _result6 = index + _i6 * (boardSize - 1);
        if (_result6 > Math.pow(boardSize, 2) - 1 || _result6 === boardSize * (numberRow + _i6) - 1) break;
        box.push(_result6);
      }
      // down-right
      for (var _i7 = 1; _i7 <= this.travelRadius; _i7 += 1) {
        var _result7 = index + _i7 * (boardSize + 1);
        if (_result7 > Math.pow(boardSize, 2) - 1 || _result7 === boardSize * (numberRow + _i7 + 1)) break;
        box.push(_result7);
      }
      return box;
    }
  }, {
    key: "attackRadiusIndex",
    value: function attackRadiusIndex(index, boardSize) {
      var box = [];
      var start = index - this.attackRadius * (boardSize + 1);
      var numberRow = Math.floor(index / boardSize);
      for (var i = 1; i <= 2 * this.attackRadius + 1; i += 1) {
        for (var j = 0; j < 2 * this.attackRadius + 1; j += 1) {
          var result = start + j;
          if (result !== index && result >= 0 && result < Math.pow(boardSize, 2) && result >= (numberRow - this.attackRadius + i - 1) * boardSize && result < (numberRow - this.attackRadius + i) * boardSize) {
            box.push(result);
          }
        }
        start += boardSize;
...
var Enemy = /*#__PURE__*/function () {
  ...
  _createClass(Enemy, null, [{
    key: "from",
    value: function from(object, boardSize) {
      var _this = this;
      this.positions = object.positions;
      this.boardSize = boardSize;
      this.plaerCharacters = object.teamPlayer.characters;
      this.players = this.positions.filter(function (el) {
        return _this.plaerCharacters.includes(el.character);
      });
      this.playerIndexes = [];
      ...
      this.enemies = this.positions.filter(function (el) {
        return _this.enemyCharacters.includes(el.character);
      });
      return null;
    }
  }, {
    key: "possibleTargets",
    value: function possibleTargets() {
      var attackIndexes = [];
      for (var i = 0; i < this.enemies.length; i += 1) {
        attackIndexes.push.apply(attackIndexes, _toConsumableArray(this.enemies[i].character.attackRadiusIndex(this.enemies[i].position, this.boardSize)));
      }
      return this.players.filter(function (el) {
        return attackIndexes.includes(el.position);
      });
    }
  }, {
    key: "damage",
    value: function damage(attacker, target) {
      return Math.max(attacker.attack - target.defence, attacker.attack * 0.1);
    }
  }, {
    key: "getMaxDamager",
    value: function getMaxDamager(target) {
      var damage = 0;
      var damager = null;
      for (var i = 0; i < this.enemies.length; i += 1) {
        var box = this.enemies[i].character.attackRadiusIndex(this.enemies[i].position, this.boardSize);
        if (box.includes(target.position)) {
          var result = this.damage(this.enemies[i].character, target.character);
          if (result > damage) {
            damage = result;
    ...
    key: "maxAttacker",
    value: function maxAttacker() {
      var result = 0;
      for (var i = 0; i < this.enemies.length; i += 1) {
        if (this.enemies[i].character.attack > result) result = this.enemies[i].character.attack;
      }
      return this.enemies.find(function (el) {
        return el.character.attack === result;
      });
    }
  }, {
    key: "minDefender",
    value: function minDefender() {
      var result = 0;
      for (var i = 0; i < this.players.length; i += 1) {
        if (this.players[i].character.defence > result) result = this.players[i].character.defence;
      }
      return this.players.find(function (el) {
        return el.character.defence === result;
      });
    }
  }, {
    key: "attackIndex",
    value: function attackIndex(enemy, indexes) {
      var _this2 = this;
      for (var i = 0; i < indexes.length; i += 1) {
        var box = enemy.attackRadiusIndex(indexes[i], this.boardSize).filter(function (el) {
          return _this2.playerIndexes.includes(el);
        });
        if (box.length !== 0) {
          return indexes[i];
        }
    ...
    value: function move() {
      ...
      for (var _i = 0; _i < this.enemies.length; _i += 1) {
        var indexesTravel = this.enemies[_i].character.travelRadiusIndex(this.enemies[_i].position, this.boardSize).filter(function (el) {
          return !posBusy.includes(el);
        });
        var attackIndex = this.attackIndex(this.enemies[_i].character, indexesTravel);
        if (attackIndex !== null) {
          return {
            enemy: this.enemies[_i],
            newPosition: attackIndex
          };
        }
      }
      var maxAttacker = this.maxAttacker();
      var box = maxAttacker.character.travelRadiusIndex(maxAttacker.position, this.boardSize).filter(function (el) {
        return !posBusy.includes(el);
      });
      var minDefender = this.minDefender();
      var difference = maxAttacker.position - minDefender.position;
      var newPosition = null;
      if (difference > 0) newPosition = Math.min.apply(Math, _toConsumableArray(box));
      if (difference < 0) newPosition = Math.max.apply(Math, _toConsumableArray(box));
      return {
        enemy: maxAttacker,
...
/* harmony import */ var _characters_Bowman__WEBPACK_IMPORTED_MODULE_17__ = __webpack_require__(/*! ./characters/Bowman */ "./src/js/characters/Bowman.js");
/* harmony import */ var _characters_Magician__WEBPACK_IMPORTED_MODULE_18__ = __webpack_require__(/*! ./characters/Magician */ "./src/js/characters/Magician.js");
/* harmony import */ var _characters_Swordsman__WEBPACK_IMPORTED_MODULE_19__ = __webpack_require__(/*! ./characters/Swordsman */ "./src/js/characters/Swordsman.js");
/* harmony import */ var _characters_Vampire__WEBPACK_IMPORTED_MODULE_20__ = __webpack_require__(/*! ./characters/Vampire */ "./src/js/characters/Vampire.js");
...
/* harmony import */ var _characters_Daemon__WEBPACK_IMPORTED_MODULE_22__ = __webpack_require__(/*! ./characters/Daemon */ "./src/js/characters/Daemon.js");
...
/* harmony import */ var _cursors__WEBPACK_IMPORTED_MODULE_25__ = __webpack_require__(/*! ./cursors */ "./src/js/cursors.js");
/* harmony import */ var _themes__WEBPACK_IMPORTED_MODULE_26__ = __webpack_require__(/*! ./themes */ "./src/js/themes.js");
/* harmony import */ var _GameState__WEBPACK_IMPORTED_MODULE_27__ = __webpack_require__(/*! ./GameState */ "./src/js/GameState.js");
/* harmony import */ var _Enemy__WEBPACK_IMPORTED_MODULE_28__ = __webpack_require__(/*! ./Enemy */ "./src/js/Enemy.js");
function _typeof(o) { "@babel/helpers - typeof"; return _typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (o) { return typeof o; } : function (o) { return o && "function" == typeof Symbol && o.constructor === Symbol && o !== Symbol.prototype ? "symbol" : typeof o; }, _typeof(o); }
...
var GameController = /*#__PURE__*/function () {
  function GameController(gamePlay, stateService) {
    ...
    this.allowedTypesPlayer = [_characters_Bowman__WEBPACK_IMPORTED_MODULE_17__["default"], _characters_Swordsman__WEBPACK_IMPORTED_MODULE_19__["default"], _characters_Magician__WEBPACK_IMPORTED_MODULE_18__["default"]];
    this.allowedTypesEnemy = [_characters_Vampire__WEBPACK_IMPORTED_MODULE_20__["default"], _characters_Undead__WEBPACK_IMPORTED_MODULE_21__["default"], _characters_Daemon__WEBPACK_IMPORTED_MODULE_22__["default"]];
  ...
  _createClass(GameController, [{
    ...
    value: function init() {
      if (this.state === null) {
        this.state = new _GameState__WEBPACK_IMPORTED_MODULE_27__["default"]();
      } else {
        Object.assign(_GameState__WEBPACK_IMPORTED_MODULE_27__["default"], this.state);
      }
      if (this.state.level === 5) {
        var score = this.calculationScore();
        this.updateScore(score);
        _GamePlay__WEBPACK_IMPORTED_MODULE_24__["default"].showMessage("\u041F\u043E\u0431\u0435\u0434\u0430!!! \n \u0422\u0435\u043A\u0443\u0449\u0438\u0439 \u0440\u0435\u0437\u0443\u043B\u044C\u0442\u0430\u0442 ".concat(score, " \n \u0420\u0435\u043A\u043E\u0440\u0434 ").concat(this.state.maxScore));
        this.removeListenerBoard();
        return;
      }
      switch (this.state.level) {
        case 1:
          this.gamePlay.drawUi(_themes__WEBPACK_IMPORTED_MODULE_26__["default"].prairie);
          break;
        case 2:
          this.gamePlay.drawUi(_themes__WEBPACK_IMPORTED_MODULE_26__["default"].desert);
          break;
        case 3:
          this.gamePlay.drawUi(_themes__WEBPACK_IMPORTED_MODULE_26__["default"].arctic);
          break;
        case 4:
          this.gamePlay.drawUi(_themes__WEBPACK_IMPORTED_MODULE_26__["default"].mountain);
          break;
        default:
          this.gamePlay.drawUi(_themes__WEBPACK_IMPORTED_MODULE_26__["default"].prairie);
          break;
      ...
      var posPlayerBusy = [];
      var maxLevel = this.state.level;
      var characterCount = this.state.level + 1;
      var validposPlayer = [];
      for (var i = 0; i < this.gamePlay.boardSize; i += 1) {
      ...
      if (this.state.teamPlayer === null) {
        this.state.teamPlayer = (0,_generators__WEBPACK_IMPORTED_MODULE_23__.generateTeam)(this.allowedTypesPlayer, maxLevel, characterCount);
      } else if (this.state.teamPlayer.characters.length < characterCount) {
        var count = characterCount - this.state.teamPlayer.characters.length;
        var result = (0,_generators__WEBPACK_IMPORTED_MODULE_23__.generateTeam)(this.allowedTypesPlayer, maxLevel, count);
        for (var _i = 0; _i < result.characters.length; _i += 1) {
          this.state.teamPlayer.characters.push(result.characters[_i]);
        }
      }
      ...
      if (this.state.positions.length === 0) {
        for (var _i2 = 0; _i2 < this.state.teamPlayer.characters.length; _i2 += 1) {
          var posPlayer = validposPlayer[Math.ceil(Math.random() * validposPlayer.length)];
          if (!posPlayerBusy.includes(posPlayer) && posPlayer !== undefined) {
            posPlayerBusy.push(posPlayer);
            this.state.positions.push(new _PositionedCharacter__WEBPACK_IMPORTED_MODULE_16__["default"](this.state.teamPlayer.characters[_i2], posPlayer));
          } else {
            _i2 -= 1;
          }
        }
      ...
      var validposEnemy = [];
      for (var _i3 = 1; _i3 < this.gamePlay.boardSize + 1; _i3 += 1) {
        validposEnemy.push(this.gamePlay.boardSize * _i3 - 2);
        validposEnemy.push(this.gamePlay.boardSize * _i3 - 1);
      }
      if (this.state.teamEnemy === null) {
        this.state.teamEnemy = (0,_generators__WEBPACK_IMPORTED_MODULE_23__.generateTeam)(this.allowedTypesEnemy, maxLevel, characterCount);
        for (var _i4 = 0; _i4 < characterCount; _i4 += 1) {
          var posEnemy = validposEnemy[Math.ceil(Math.random() * validposEnemy.length)];
          if (!posPlayerBusy.includes(posEnemy) && posEnemy !== undefined) {
            posPlayerBusy.push(posEnemy);
            this.state.positions.push(new _PositionedCharacter__WEBPACK_IMPORTED_MODULE_16__["default"](this.state.teamEnemy.characters[_i4], posEnemy));
          } else {
            _i4 -= 1;
          }
        }
    ...
    value: function onCellClick(index) {
      ...
      var teamPlayer = ['bowman', 'swordsman', 'magician'];
      if (this.state.turn === 'enemy') return;
      if (this.gamePlay.cells[index].querySelector('.character') && teamPlayer.includes(this.state.positions.find(function (el) {
        return el.position === index;
      }).character.type)) {
        // Ошибка
        var lastClick = document.querySelector('.selected-yellow');
        if (lastClick !== null) {
          var indexLastClick = this.gamePlay.cells.findIndex(function (el) {
            return el === lastClick;
          });
          this.gamePlay.deselectCell(indexLastClick);
        }
        this.gamePlay.selectCell(index);
      } else if (document.querySelector('.selected-yellow') && document.querySelector('.selected-green')) {
        this.state.turn = 'enemy';
        var _lastClick = document.querySelector('.selected-yellow');
        var _indexLastClick = this.gamePlay.cells.findIndex(function (el) {
          return el === _lastClick;
        });
        var newClick = document.querySelector('.selected-green');
        var indexNewClick = this.gamePlay.cells.findIndex(function (el) {
          return el === newClick;
        });
        var positionedCharacter = this.state.positions.find(function (el) {
          return el.position === _indexLastClick;
        });
        positionedCharacter.position = indexNewClick;
        this.gamePlay.deselectCell(_indexLastClick);
        this.onCellLeave(indexNewClick);
        this.gamePlay.redrawPositions(this.state.positions);
        this.turnEnemy();
      } else if (document.querySelector('.selected-red')) {
        this.state.turn = 'enemy';
        var indexAttacker = this.gamePlay.cells.findIndex(function (el) {
          return el === document.querySelector('.selected-yellow');
        });
        var attacker = this.state.positions.find(function (el) {
          return el.position === indexAttacker;
        }).character;
        var indexTarget = this.gamePlay.cells.findIndex(function (el) {
          return el === document.querySelector('.selected-red');
        });
        var targetPosition = this.state.positions.find(function (el) {
          return el.position === indexTarget;
        });
        var target = targetPosition.character;
        var damage = Math.max(attacker.attack - target.defence, attacker.attack * 0.1);
        target.health -= damage;
        if (target.health <= 0) {
          this.state.positions = this.state.positions.filter(function (el) {
            return el !== targetPosition;
          });
          this.state.teamEnemy.characters = this.state.teamEnemy.characters.filter(function (el) {
            return el !== target;
          });
        }
        this.gamePlay.deselectCell(indexAttacker);
        this.onCellLeave(indexTarget);
        this.gamePlay.showDamage(indexTarget, damage).then(function () {
          if (_this.state.teamEnemy.characters.length === 0) {
            for (var i = 0; i < _this.state.positions.length; i += 1) {
              _this.state.positions[i].character.levelUp();
            }
            _this.state.level += 1;
            _this.state.teamEnemy = null;
            _this.state.turn = 'player';
            _this.init();
          } else {
            _this.gamePlay.redrawPositions(_this.state.positions);
            _this.turnEnemy();
          }
        });
      } else {
        _GamePlay__WEBPACK_IMPORTED_MODULE_24__["default"].showError('Недоступное действие');
      }
    }
  }, {
    key: "onCellEnter",
    value: function onCellEnter(index) {
      ...
      this.gamePlay.setCursor(_cursors__WEBPACK_IMPORTED_MODULE_25__["default"].notallowed);
      if (this.state.turn === 'enemy') return;
      var boardSize = this.gamePlay.boardSize;
      var playerTeam = ['bowman', 'swordsman', 'magician'];
      var enemyTeam = ['vampire', 'undead', 'daemon'];
      if (this.state.positions.find(function (el) {
        return el.position === index;
      })) {
        var _this$state$positions = this.state.positions.find(function (el) {
            return el.position === index;
          }),
          character = _this$state$positions.character;
        var message = character.showInformation();
        this.gamePlay.showCellTooltip(message, index);
        if (playerTeam.includes(this.state.positions.find(function (el) {
          return el.position === index;
        }).character.type)) {
          this.gamePlay.setCursor(_cursors__WEBPACK_IMPORTED_MODULE_25__["default"].pointer);
        }
        if (document.querySelector('.selected-yellow') !== null && enemyTeam.includes(this.state.positions.find(function (el) {
          return el.position === index;
        }).character.type)) {
          var lastClick = document.querySelector('.selected-yellow');
          var indexLastClick = this.gamePlay.cells.findIndex(function (el) {
          ...
          var lastCharacter = this.state.positions.find(function (el) {
            return el.position === indexLastClick;
          }).character;
          if (lastCharacter.attackRadiusIndex(indexLastClick, boardSize).some(function (e) {
            return e === index;
          })) {
            this.gamePlay.setCursor(_cursors__WEBPACK_IMPORTED_MODULE_25__["default"].crosshair);
            this.gamePlay.selectCell(index, 'red');
          }
        }
      }
      if (document.querySelector('.selected-yellow') !== null && !this.gamePlay.cells[index].querySelector('.character')) {
        var _lastClick2 = document.querySelector('.selected-yellow');
        var _indexLastClick2 = this.gamePlay.cells.findIndex(function (el) {
          return el === _lastClick2;
        });
        var _this$state$positions2 = this.state.positions.find(function (el) {
            return el.position === _indexLastClick2;
          }),
          _character = _this$state$positions2.character;
        if (_character.travelRadiusIndex(_indexLastClick2, boardSize).some(function (e) {
          return e === index;
        })) {
          this.gamePlay.setCursor(_cursors__WEBPACK_IMPORTED_MODULE_25__["default"].pointer);
    ...
    value: function onNewGame() {
      ...
      this.init();
      return _GamePlay__WEBPACK_IMPORTED_MODULE_24__["default"].showMessage('Новая игра');
    }
  }, {
    key: "onSaveGame",
    value: function onSaveGame() {
      this.stateService.save(this.state);
      return _GamePlay__WEBPACK_IMPORTED_MODULE_24__["default"].showMessage('Игра сохранена');
    }
  }, {
    key: "onLoadGame",
    value: function onLoadGame() {
      if (!this.stateService.load()) {
        return _GamePlay__WEBPACK_IMPORTED_MODULE_24__["default"].showMessage('Нет сохраненной игры');
      }
      _GameState__WEBPACK_IMPORTED_MODULE_27__["default"].from(this.stateService.load());
      Object.assign(this.state, _GameState__WEBPACK_IMPORTED_MODULE_27__["default"]);
      this.init();
      this.removeListenerBoard();
      this.addListenersBoard();
      return _GamePlay__WEBPACK_IMPORTED_MODULE_24__["default"].showMessage('Игра загружена');
    }
  }, {
    key: "turnEnemy",
    value: function turnEnemy() {
      var _this2 = this;
      _Enemy__WEBPACK_IMPORTED_MODULE_28__["default"].from(this.state, this.gamePlay.boardSize);
      var possibleTargets = _Enemy__WEBPACK_IMPORTED_MODULE_28__["default"].possibleTargets();
      if (possibleTargets.length !== 0) {
        var target = null;
        var damage = 0;
        for (var i = 0; i < possibleTargets.length; i += 1) {
          var maxDamager = _Enemy__WEBPACK_IMPORTED_MODULE_28__["default"].getMaxDamager(possibleTargets[i]);
        ...
        if (target.character.health <= 0) {
          ...
          this.state.teamPlayer.characters = this.state.teamPlayer.characters.filter(function (el) {
            return el !== target.character;
          });
        }
        this.gamePlay.showDamage(target.position, damage).then(function () {
          if (_this2.state.teamPlayer.characters.length === 0) {
            var score = _this2.calculationScore();
            _this2.updateScore(score);
            _GamePlay__WEBPACK_IMPORTED_MODULE_24__["default"].showMessage("\u0418\u0433\u0440\u0430 \u043E\u043A\u043E\u043D\u0447\u0435\u043D\u0430 \n \u0422\u0435\u043A\u0443\u0449\u0438\u0439 \u0440\u0435\u0437\u0443\u043B\u044C\u0442\u0430\u0442 ".concat(score, " \n \u0420\u0435\u043A\u043E\u0440\u0434 ").concat(_this2.state.maxScore));
            _this2.removeListenerBoard();
          } else {
            _this2.gamePlay.redrawPositions(_this2.state.positions);
          }
          _this2.state.turn = 'player';
        });
      } else {
        var moveItem = _Enemy__WEBPACK_IMPORTED_MODULE_28__["default"].move();
        this.state.positions.find(function (el) {
          return el === moveItem.enemy;
        }).position = moveItem.newPosition;
        this.gamePlay.redrawPositions(this.state.positions);
        this.state.turn = 'player';
      }
    }
  }, {
    key: "addListenersBoard",
    value: function addListenersBoard() {
      this.gamePlay.addCellEnterListener(this.onCellEnter.bind(this));
      this.gamePlay.addCellLeaveListener(this.onCellLeave.bind(this));
      this.gamePlay.addCellClickListener(this.onCellClick.bind(this));
    }
  }, {
    key: "addListenersGame",
    value: function addListenersGame() {
      this.gamePlay.addNewGameListener(this.onNewGame.bind(this));
      this.gamePlay.addSaveGameListener(this.onSaveGame.bind(this));
      this.gamePlay.addLoadGameListener(this.onLoadGame.bind(this));
    }
  }, {
    key: "removeListenerBoard",
    value: function removeListenerBoard() {
      this.gamePlay.cellClickListeners = [];
      this.gamePlay.cellEnterListeners = [];
      this.gamePlay.cellLeaveListeners = [];
    }
  }, {
    key: "calculationScore",
    value: function calculationScore() {
      var result = 0;
    ...
    value: function updateScore(score) {
      var state = this.stateService.load();
      if (state !== null) {
        ...
        var element = new _GameState__WEBPACK_IMPORTED_MODULE_27__["default"]();
...
/* harmony import */ var core_js_modules_web_dom_collections_iterator_js__WEBPACK_IMPORTED_MODULE_18__ = __webpack_require__(/*! core-js/modules/web.dom-collections.iterator.js */ "./node_modules/core-js/modules/web.dom-collections.iterator.js");
/* harmony import */ var core_js_modules_web_dom_collections_iterator_js__WEBPACK_IMPORTED_MODULE_18___default = /*#__PURE__*/__webpack_require__.n(core_js_modules_web_dom_collections_iterator_js__WEBPACK_IMPORTED_MODULE_18__);
/* harmony import */ var _utils__WEBPACK_IMPORTED_MODULE_19__ = __webpack_require__(/*! ./utils */ "./src/js/utils.js");
function _typeof(o) { "@babel/helpers - typeof"; return _typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (o) { return typeof o; } : function (o) { return o && "function" == typeof Symbol && o.constructor === Symbol && o !== Symbol.prototype ? "symbol" : typeof o; }, _typeof(o); }
function _toConsumableArray(arr) { return _arrayWithoutHoles(arr) || _iterableToArray(arr) || _unsupportedIterableToArray(arr) || _nonIterableSpread(); }
function _nonIterableSpread() { throw new TypeError("Invalid attempt to spread non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method."); }
function _iterableToArray(iter) { if (typeof Symbol !== "undefined" && iter[Symbol.iterator] != null || iter["@@iterator"] != null) return Array.from(iter); }
  ...
  _createClass(GamePlay, [{
    ...
    value: function drawUi(theme) {
      ...
      for (var i = 0; i < Math.pow(this.boardSize, 2); i += 1) {
        ...
        cellEl.classList.add('cell', 'map-tile', "map-tile-".concat((0,_utils__WEBPACK_IMPORTED_MODULE_19__.calcTileType)(i, this.boardSize)));
    ...
    value: function redrawPositions(positions) {
      ...
      try {
        for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
          ...
          healthIndicatorEl.classList.add('health-level-indicator', "health-level-indicator-".concat((0,_utils__WEBPACK_IMPORTED_MODULE_19__.calcHealthLevel)(position.character.health)));
...
/* harmony import */ var _Character__WEBPACK_IMPORTED_MODULE_10__ = __webpack_require__(/*! ./Character */ "./src/js/Character.js");
...
var PositionedCharacter = /*#__PURE__*/_createClass(function PositionedCharacter(character, position) {
  ...
  if (!(character instanceof _Character__WEBPACK_IMPORTED_MODULE_10__["default"])) {
...
/* harmony import */ var _Character__WEBPACK_IMPORTED_MODULE_12__ = __webpack_require__(/*! ../Character */ "./src/js/Character.js");
function _typeof(o) { "@babel/helpers - typeof"; return _typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (o) { return typeof o; } : function (o) { return o && "function" == typeof Symbol && o.constructor === Symbol && o !== Symbol.prototype ? "symbol" : typeof o; }, _typeof(o); }
...
var Bowman = /*#__PURE__*/function (_Character) {
  ...
  function Bowman(level, attack, defence, health, type, travelRadius, attackRadius) {
    var _this;
    _classCallCheck(this, Bowman);
    _this = _callSuper(this, Bowman, [level, attack, defence, health, type, travelRadius, attackRadius]);
    _this.level = 1;
    _this.attack = 25;
    _this.defence = 25;
    _this.health = 50;
    _this.type = 'bowman';
    _this.travelRadius = 2;
    _this.attackRadius = 2;
    if (level > 1) {
...
}(_Character__WEBPACK_IMPORTED_MODULE_12__["default"]);
...
var Daemon = /*#__PURE__*/function (_Character) {
  ...
  function Daemon(level, attack, defence, health, type, travelRadius, attackRadius) {
    var _this;
    _classCallCheck(this, Daemon);
    _this = _callSuper(this, Daemon, [level, attack, defence, health, type, travelRadius, attackRadius]);
    _this.level = 1;
    _this.attack = 10;
    _this.defence = 10;
    _this.health = 50;
    _this.type = 'daemon';
    _this.travelRadius = 1;
    _this.attackRadius = 4;
    if (level > 1) {
      for (var i = 1; i < level; i += 1) {
        _this.levelUp();
      }
...
var Magician = /*#__PURE__*/function (_Character) {
  ...
  function Magician(level, attack, defence, health, type, travelRadius, attackRadius) {
    var _this;
    _classCallCheck(this, Magician);
    _this = _callSuper(this, Magician, [level, attack, defence, health, type, travelRadius, attackRadius]);
    _this.level = 1;
    _this.attack = 10;
    _this.defence = 40;
    _this.health = 50;
...
var Swordsman = /*#__PURE__*/function (_Character) {
  ...
  function Swordsman(level, attack, defence, health, type, travelRadius, attackRadius) {
    var _this;
    _classCallCheck(this, Swordsman);
    _this = _callSuper(this, Swordsman, [level, attack, defence, health, type, travelRadius, attackRadius]);
    _this.level = 1;
    _this.attack = 40;
    _this.defence = 10;
    _this.health = 50;
    _this.type = 'swordsman';
    _this.travelRadius = 4;
    _this.attackRadius = 1;
    if (level > 1) {
      for (var i = 1; i < level; i += 1) {
        _this.levelUp();
      }
...
var Undead = /*#__PURE__*/function (_Character) {
  ...
  function Undead(level, attack, defence, health, type, travelRadius, attackRadius) {
    var _this;
    _classCallCheck(this, Undead);
    _this = _callSuper(this, Undead, [level, attack, defence, health, type, travelRadius, attackRadius]);
    _this.level = 1;
    _this.attack = 40;
    _this.defence = 10;
    _this.health = 50;
...
var Vampire = /*#__PURE__*/function (_Character) {
  ...
  function Vampire(level, attack, defence, health, type, travelRadius, attackRadius) {
    var _this;
    _classCallCheck(this, Vampire);
    _this = _callSuper(this, Vampire, [level, attack, defence, health, type, travelRadius, attackRadius]);
    _this.level = 1;
    _this.attack = 25;
    _this.defence = 25;
    _this.health = 50;
...
/* harmony import */ var core_js_modules_web_dom_collections_iterator_js__WEBPACK_IMPORTED_MODULE_15__ = __webpack_require__(/*! core-js/modules/web.dom-collections.iterator.js */ "./node_modules/core-js/modules/web.dom-collections.iterator.js");
/* harmony import */ var core_js_modules_web_dom_collections_iterator_js__WEBPACK_IMPORTED_MODULE_15___default = /*#__PURE__*/__webpack_require__.n(core_js_modules_web_dom_collections_iterator_js__WEBPACK_IMPORTED_MODULE_15__);
/* harmony import */ var _Team__WEBPACK_IMPORTED_MODULE_16__ = __webpack_require__(/*! ./Team */ "./src/js/Team.js");
function _typeof(o) { "@babel/helpers - typeof"; return _typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (o) { return typeof o; } : function (o) { return o && "function" == typeof Symbol && o.constructor === Symbol && o !== Symbol.prototype ? "symbol" : typeof o; }, _typeof(o); }
...
function characterGenerator(allowedTypes, maxLevel) {
  var characterRandom, levelRandom;
  return _regeneratorRuntime().wrap(function characterGenerator$(_context) {
    while (1) switch (_context.prev = _context.next) {
      case 0:
        // TODO: write logic here
        characterRandom = Math.round(Math.random() * (allowedTypes.length - 1));
        levelRandom = Math.ceil(Math.random() * maxLevel);
        _context.next = 4;
        return new allowedTypes[characterRandom](levelRandom);
      case 4:
      case "end":
...
function generateTeam(allowedTypes, maxLevel, characterCount) {
  ...
  return new _Team__WEBPACK_IMPORTED_MODULE_16__["default"](characters);
}
...
function calcTileType(index, boardSize) {
  ...
  if (index > boardSize * (boardSize - 1) && index < Math.pow(boardSize, 2) - 1) return 'bottom';
  return 'center';
}
